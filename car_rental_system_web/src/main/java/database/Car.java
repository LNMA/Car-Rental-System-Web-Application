...
public class Car {
	...
	try {
	    PreparedStatement preparedStatement = connection.prepareStatement("select * from car left join car_sedan on car.idCar = car_sedan.idCar left join car_van on car.idCar = car_van.idCar order by car.idCar ");
	    ResultSet resultSet = preparedStatement.executeQuery();
    ...
    public ArrayList<CarReference> getAllAvailableCar() {
	ArrayList<CarReference> all = new ArrayList<>();
	try {
	    PreparedStatement preparedStatement = connection.prepareStatement("select * from car left join car_sedan on car.idCar = car_sedan.idCar left join car_van on car.idCar = car_van.idCar where car.status='Available'");
	    ResultSet resultSet = preparedStatement.executeQuery();
	    while (resultSet.next()) {
		all.add(new CarReference(resultSet.getString(1), resultSet.getTimestamp(2), resultSet.getString(3), resultSet.getString(4), resultSet.getString(5), resultSet.getInt(6), resultSet.getInt(7), resultSet.getString(8), resultSet.getString(9), resultSet.getDouble(10), resultSet.getDouble(11), resultSet.getString(12), resultSet.getDouble(14), resultSet.getDouble(16), resultSet.getDouble(17)));
	    }
